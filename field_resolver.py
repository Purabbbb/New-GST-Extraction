# NOTE: this file previously had an entire earlier draft of itself
# (lines 1-1274 of the original) sitting inertly as an unassigned
# triple-quoted string literal at the top -- Python parses it, discards
# the value, and moves on, so it was never a functional bug, just 100KB
# of dead weight that made the file confusing to navigate/diff. Removed.
...
# A GSTIN's characters 3-12 (0-indexed 2:12) ARE the entity's PAN by
# construction. If a "PAN :" value found next to a GSTIN candidate matches
# that embedded PAN, the candidate is confirmed to belong to this specific
# block -- this holds even when two-column layouts (e.g. "Supplier" |
# "Recipient" side by side) get flattened into a scrambled 1-D line order,
# because it doesn't depend on distance-to-heading at all.
_PAN_TOKEN_RE = re.compile(
    r'\bPAN\b\s*[:.\-]?\s*([A-Z]{5}[0-9]{4}[A-Z])', re.IGNORECASE
)


def _embedded_pan(gstin: str) -> str:
    return gstin[2:12].upper() if gstin and len(gstin) >= 12 else ''


def _pan_confirms_gstin(lines: list[str], gstin: str, line_idx: int, radius: int = 2) -> bool:
    """True if a PAN matching what's encoded in `gstin` appears within
    `radius` lines of `line_idx` (covers same-line and adjacent-line PAN
    mentions, which is how both this invoice's supplier and recipient
    blocks lay them out)."""
    embedded = _embedded_pan(gstin)
    if not embedded:
        return False
    lo = max(0, line_idx - radius)
    hi = min(len(lines), line_idx + radius + 1)
    for candidate_line in lines[lo:hi]:
        for m in _PAN_TOKEN_RE.finditer(candidate_line):
            if m.group(1).upper() == embedded:
                return True
    return False
...
def _resolve_gstin_by_context(
    ...
    *,
    ...
    Improvement over v2: a candidate whose embedded PAN is confirmed by a
    nearby literal "PAN :" value gets a large score boost. This is a
    same-block confirmation signal that survives OCR column-interleaving,
    unlike pure line-distance-to-heading (see _pan_confirms_gstin).
    """
    sorted_kws = sorted(keywords, key=len, reverse=True)
    ...
    for i, line in enumerate(lines):
        ...
        for offset in range(0, min(window, len(lines) - i)):
            ...
            for m in _GSTIN_RE.finditer(candidate_line):
                ...
                if _pan_confirms_gstin(lines, gstin, i + offset):
                    score = min(1.0, score + 0.30)
                    method += ' +pan-confirmed'
                c = _Candidate(
                    value=gstin,
                    score=score,
                    keyword=line,
